...
from sdn_hybrid_lb.utils.models import BackendServer, Capacity, FlowBinding, VipConfig
...
class HybridLoadBalancer:
    ...
    - Round Robin / Smooth Weighted Round Robin for fast flow placement.
    - Genetic Algorithm for periodic long-term weight optimisation.
    - Overload gating for *new* flows only.
            ...
            self.backends.append(
                BackendServer(
                    ...
                    capacity=Capacity(
                        ...
                        max_connections=int(cap.get("max_connections", 1000)),
                    ),
                )
            )
    ...
    def _is_eligible(self, backend: BackendServer) -> bool:
        ...
        if m.bw_util is not None and m.bw_util >= thr.bw:
            return False
        if backend.connection_util() >= thr.conn:
            return False
        return True
    ...
    def status(self) -> Dict:
        ...
        return {
            ...
            "controller": {
                ...
                "last_ga_run": self._last_ga_run,
            },
            "weights": dict(self._weights),
            "backends": [b.as_dict() for b in self.backends],
            "eligible_backends": [b.name for b in self.backends if self._is_eligible(b)],
