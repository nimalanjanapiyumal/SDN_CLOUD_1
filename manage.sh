...
start_bg(){
  ...
  if kill -0 "$(cat "$pidfile")" 2>/dev/null; then
    echo "[OK] Started $name (PID $(cat "$pidfile"))"
...
status_bg(){
  ...
  if [[ -f "$pidfile" ]] && kill -0 "$(cat "$pidfile")" 2>/dev/null; then
    echo "[INFO] $name running (PID $(cat "$pidfile"))"
  else
  ...
  controller)
    case "$SUB" in
      ...
      start)
        fix_perms
        cleanup_ports
        if [[ -f "$ROOT/.venv-controller/bin/activate" ]]; then
          # shellcheck disable=SC1091
          source "$ROOT/.venv-controller/bin/activate"
        fi
        python "$ROOT/scripts/check_controller_env.py"
        start_bg controller "cd '$ROOT/vm-a1-controller' && source '$ROOT/.venv-controller/bin/activate' && bash ./run_controller.sh"
        ;;
